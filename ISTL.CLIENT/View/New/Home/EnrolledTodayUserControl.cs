...
{
    public partial class EnrolledTodayUserControl : ViewUserControl
    {
        private int totalCount;
        private DbUserManager dbUserManager;
        public EnrolledTodayUserControl()
        {
            InitializeComponent();            
        }
        ...
        protected override void OnLoad()
        {
            ...
            totalCount = ((EnrolledTodayController)controller).GetEnrolledTodayCount();
            OnSearch(0);
        }
        ...
        private void ShowEnrolledTodayList(List<EnrollmentDto> list)
        {
            dgvList.Rows.Clear();
            totalCount = (((EnrolledTodayController)controller).RecordCount >= 0) ? ((EnrolledTodayController)controller).RecordCount : 0;
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                dgvList.Rows.Add(index, list[i].profile?.referenceNo, list[i].profile?.fullName, list[i].profile?.dateOfBirth,
                    list[i].profile?.gender, createdByName, list[i].profile?.hash, AppUtils.AppUtils.GetRecordStatus(list[i].profile?.status), list[i].profile?.status, "VIEW");
            }
        }
        ...
        private void OnSearch(int pos)
        {
            ...
            List<EnrollmentDto> list = ((EnrolledTodayController)controller).GetEnrolledTodayData(whereClause, pos);

            if (list == null) return;
            else ShowEnrolledTodayList(list);
        }
        ...
        private void dgvList_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 9)
            {
                int enrolStatus = Convert.ToInt32(dgvList.CurrentRow.Cells[8]?.Value?.ToString());
                if (enrolStatus == Globals.RecordState.NEW)
                {
                    CustomMessageBox.ShowMessage("SNSOP TOOLS", "You can't view/edit this profile as it has been submitted for upload already");
                    return;
                }
                string hash = dgvList.CurrentRow.Cells[6]?.Value?.ToString();
                ((EnrolledTodayController)controller).GetDataByHash(hash);
            }
        }
        ...
        {
            int failedRecordTotal = totalCount;
            if (failedRecordTotal % 10 != 0) position = (failedRecordTotal / 10) * 10;
            else if (failedRecordTotal % 10 == 0) position = ((failedRecordTotal / 10) - 1) * 10;
            OnSearch(position);
        }
        ...
        {
            ((EnrolledTodayController)controller).GoBacktoDashboard();
        }
    }
}
