...
{
    public partial class FailedUploadUserControl : ViewUserControl
    {
        private int totalCount;
        private DbUserManager dbUserManager;
        public FailedUploadUserControl()
        {
            InitializeComponent();
        }
        ...
        protected override void OnLoad()
        {
            ...
            totalCount = ((FailedUploadController)controller).GetFailedUploadCount();
            OnSearch(0);
        }
        ...
        {
            dgvFailedList.Rows.Clear();
            totalCount = (((FailedUploadController)controller).RecordCount >= 0) ? ((FailedUploadController)controller).RecordCount : 0;
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                dgvFailedList.Rows.Add(index, list[i].profile?.referenceNo, list[i].profile?.fullName, list[i].profile?.dateOfBirth,
                    list[i].profile?.gender, createdByName, list[i].profile?.errorMsgFromServer, "VIEW", list[i].profile?.hash, list[i].profile?.id);
            }
        }
        ...
        private void OnSearch(int pos)
        {
            ...
            List<EnrollmentDto> list = ((FailedUploadController)controller).GetFailedData(whereClause, pos);

            if (list == null) return;
            else ShowDraftList(list);
        }

        private void dgvFailedList_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex < 0)
            {
                return;
            ...
            if (e.ColumnIndex == 7)
            {
                string hash = dgvFailedList.CurrentRow.Cells[8]?.Value?.ToString();
                ((FailedUploadController)controller).GetDataByHash(hash);
            }
        }
        ...
        {
            ((FailedUploadController)controller).GoBacktoDashboard();
        }
    }
}
