...
{
    public partial class NotEntryUploadPendingUserControl : ViewUserControl
    {
        private Logger logger = LogManager.GetCurrentClassLogger();
        public NotEntryUploadPendingUserControl()
        {
            InitializeComponent();
        ...
        private void OnSearch(int startIndex)
        {
            ...
            List<NotEntryDto> list = ((NotEntryUploadPendingController)controller).GetUploadPendingData(whereClause, startIndex);
        ...
        {
            dgvList.Rows.Clear();
            totalCount = (((NotEntryUploadPendingController)controller).RecordCount >= 0) ? ((NotEntryUploadPendingController)controller).RecordCount : 0;
            ...
            for (int i = 0; i < list?.Count; i++)
            {
                ...
                dgvList.Rows.Add(index, list[i].referenceNo, list[i].accusedName, reason, caseType, list[i].mobileNo, "Edit", list[i].createdBy);
        ...
        {
            ((NotEntryUploadPendingController)controller).GoBackToDashboard();
        }
    }
}
